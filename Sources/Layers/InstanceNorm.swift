...
open class InstanceNorm: NetworkLayer {
    ...
    open override func updatedCheckpoint(device: MTLDevice) {
        ...
        scaleBuffer.contents().copyMemory(from: network.parameterLoader.loadWeights(for: id, modifier: InstanceNorm.scaleModifier, size: outputSize.f),
                                          byteCount: max(4, outputSize.f) * Constants.FloatSize)
        shiftBuffer.contents().copyMemory(from: network.parameterLoader.loadWeights(for: id, modifier: InstanceNorm.shiftModifier, size: outputSize.f),
                                          byteCount: max(4, outputSize.f) * Constants.FloatSize)
    }
