    ...
    func optimize(graph: TensorflowGraph) {
        for node in graph.nodes {
            if node.nodeDef.op.isTFConvOp,
                let out = node.outgoingNodes() as? [TensorflowNode], out.count == 1,
                let biasAdd = out.first {
                if biasAdd.nodeDef.op.isTFBiasAddOp,
                    let biasVar = (biasAdd.incomingNodes() as? [TensorflowNode])?.filter({ $0.nodeDef.op.isTFVariableV2Op }).first {
